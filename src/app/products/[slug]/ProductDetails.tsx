...
const ProductDetails = ({ product }: ProductDetailsProps) => {
  ...
  const selectedOptionsMedia = product.productOptions?.flatMap(
    (productOption) => {
      const selectedChoice = productOption?.choices?.find(
        (choice) =>
          choice.description === selectedOptions[productOption.name || ""],
      );
    ...
    <div className="flex flex-col gap-10 md:flex-row lg:gap-20">
      ...
      <div className="basis-3/5 space-y-5">
        <div className="space-y-2.5">
          <h1 className="text-3xl font-bold lg:text-3xl">{product.name}</h1>
          {product.brand && (
